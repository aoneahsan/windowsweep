...
| 8 | On a Windows 11 machine or VM: `npx windowsweep --self-test` (expect 114/114), `npx windowsweep --dry-run --all --yes`, then one real `npx windowsweep --all --yes`; send the agent the build number and the report | needs a Windows 11 machine | `remaining-work.md` RW-021 | Not started |
