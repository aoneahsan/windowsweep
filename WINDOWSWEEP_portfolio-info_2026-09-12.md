...
Current Version Reviewed: `1.2.0` on npm (the same engine as `main`); the desktop app is at `desktop-v1.1.0`, with `desktop-v1.2.0` in preparation
Last Portfolio Update: 2026-09-12
Next Eligible Update After: 2026-09-19
...
| Public URL (Live) | https://windowsweep.aoneahsan.com — the product site and canonical homepage, **live since 2026-09-08** on Firebase Hosting (probed 200 on 2026-09-12). Home, `/download`, `/changelog`, `/privacy`, `/contact`, `/account`, a readable sitemap and an RSS feed |
...
Honest framing: this is Windows-only (`os: ["win32"]`), it deletes rather than archives, there is no undo for caches, and correctness rests on a 156-check self-test plus dry-runs and real runs rather than a conventional unit-test suite. Sections that need Administrator rights are skipped with the exact command when the console is not elevated, and the admin, Windows 11 and Scheduled Task paths are still on the verification list. The command-line tool sends nothing; the desktop window and the website send usage analytics and crash reports with no opt-out, stated in one line on their first screen and on the privacy page.
...
| Desktop app | Tauri 2 + React 19 + Vite 8 + Tailwind v4 + React Aria; a Rust shell with an argument allowlist and 18 tests; NSIS and MSI installers, minisign-signed, with an in-app updater; Supabase Auth (Google) and PostgREST for optional settings sync |
| Product site | Vite 8 + React 19 + TanStack Router + Supabase on the same project as the desktop app; Firebase Hosting for hosting and GA4 only; Capacitor wired without a platform folder |
...
| Documentation | In-repo `docs/` (14 pages) plus an AI integration guide, a Docusaurus site on GitHub Pages, and the product site |
| Scheduling | Weekly Scheduled Task installer (Sunday 03:00, safe batch) and a PowerShell profile alias |
| Privacy | The command-line tool makes zero network calls — no telemetry, no analytics, no update check; feedback is a user-initiated browser page and local zip bundles. The desktop window and the website send usage analytics (GA4, Amplitude, Clarity with all text masked, Sentry) with no opt-out, never a file path, a user name or the contents of anything |
| Verification | `--self-test` (156 checks: syntax, ASCII-only source, the protection lists, a real junction fixture, a 400-character path, the dry-run guarantee, keep-newest, prompt asymmetry, pure helpers, export fixtures, and the 1.2.0 contract - exclusion at the chokepoint and its children, dry-run/real parity, the scan developer flag); CI on both PowerShell hosts; PSScriptAnalyzer; the desktop app's typecheck, lint, build, clippy and cargo test in CI |
...
- **Self-test** — 156 checks that prove the guards on the user's own machine before they trust it.
...
- **A desktop window over the same engine** — the Reclaim Map (a treemap of what is reclaimable, shaded by idle age), a safe run in one press, per-section progress, a picker for the interactive sections, run history and reports, a weekly-schedule switch, tile-click exclusions, one theme control with ten appearance axes, and an in-app updater; optional Google sign-in syncs settings and run history between machines.
- **A product website** — the download page with sizes and checksums, the changelog as an RSS feed, the privacy page that states both facts at the same size, signed-in contact requests, and an account page.
...
- **The desktop app's design is a click dummy that owns the words** — every screen was approved as static HTML first, and a parity check compares the shipped window against it screenshot by screenshot; the check once found the primary button offering to reclaim 0 B above a ladder promising 39 GB.
...
- **No automated test suite beyond `--self-test`.** Correctness rests on 156 fixture-based checks, dry-runs, and real runs on Windows 10; the desktop app adds typecheck, lint, build, clippy and 18 Rust tests.
...
- **The documentation site answers over HTTP only** until GitHub issues its certificate; the product site is HTTPS.
- **The desktop window and the website send usage analytics with no opt-out**, stated in one line; the command line sends nothing.
