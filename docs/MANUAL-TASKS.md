...
> Last updated: 2026-09-05 (rows 14 and 17 completed by the owner; row 22 added - the Build Tools UAC click)
...
| 22 | Install the C++ build tools so the desktop app can link locally - one UAC click: `winget install --id Microsoft.VisualStudio.2022.BuildTools --override "--quiet --wait --add Microsoft.VisualStudio.Workload.VCTools --includeRecommended"` (about 5 GB). rustup is already installed per-user; without the MSVC linker nothing Rust-side builds here and the Tauri half stays CI-verified only | an agent session must never raise a UAC prompt | plan section 18, Block O | Not started |
