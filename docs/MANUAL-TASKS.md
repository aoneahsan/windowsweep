...
| 23 | **Say yes (or no) to creating the Firebase project** for the desktop app's sign-in and sync. Everything else is written and committed: `desktop/firebase/firestore.rules` (deny by default, a field allowlist on both documents), `firestore.indexes.json` and `firebase.json`. Once you say go, an agent runs `firebase projects:create`, enables the Google provider and deploys `--only firestore:rules,firestore:indexes` - no console visit needed. 🔴 It is a row rather than an agent action because it creates a new cloud project on your Google account, which is outward-facing and awkward to undo | creating a cloud project on your account is your call, not a session's | `desktop/firebase/README.md`; needed together with rows 15 and 16 | Not started |
