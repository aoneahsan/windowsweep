...
|---|---|---|---|---|
| 13 | 2026-09-25 | HEAD `d3475bc`, desktop 1.3.0 | **CLEAN** - every TASK-013 surface matches the dummy (11 states, 44 pairs at 1440 and 760, light and dark; all 34 sync strings verbatim), D-64 and the Scan / Dry-run states hold, both version surfaces read 1.3.0, zero app defects. Run on an ISOLATED WebView2 profile (`WEBVIEW2_USER_DATA_FOLDER`) - the owner's own profile untouched - as a signed-in NON-admin, behind the two-wire guard: 14 engine invocations, all `--list`, `--scan` or `--dry-run`, Reclaim pressed zero times. Settings was out of this round's scope; its team-voice change (D44) gets its own check before the tag | [`rounds/round-13.md`](rounds/round-13.md) |
| 14 | 2026-09-25 | HEAD `8f3a03f` (verdict holds at `bd1b7cd`, no desktop change), desktop 1.3.0 | **2 DEFECTS, both older than the change under review** - D-66: Settings has no status-bar note, where the dummy says *settings sync when you are signed in* (withheld since round 8 until sync was wired; TASK-013 wired it). D-67: an app-only panel on the About tab (three sentences, **Source** and **Support this work**), never in the dummy and never judged by a round. The team-voice change itself (D44) is clean: heading and note word for word, the same 14 roster products in the same order, the self-exclusion audit identical on both sides with its control. Home and Account signed out hold round 13; both version surfaces 1.3.0. Signed out throughout, isolated WebView2 profile, two-wire guard: 8 engine calls, all `--list --json`, Reclaim, Scan and Dry-run pressed zero times | [`rounds/round-14.md`](rounds/round-14.md) |
| 15 | 2026-09-25 | HEAD `5934008`, desktop 1.3.0 | **CLEAN** - D-66 and D-67 closed: the About panel (D47, the owner kept its words) matches the dummy in words and pixels at 1440 and 760, the Settings status note shows on all five tabs at 1440 on both sides and both hide it at 760 by the same rule; the dummy's buttons go pending, done, idle; the app's go pending, then idle, because the guard refused every opener call (proved on both wires before each press) - nothing reached a browser. Home and Account signed out hold round 13; both version surfaces 1.3.0. Signed out throughout, isolated WebView2 profile: 6 engine calls, all `--list --json`, Reclaim, Scan and Dry-run pressed zero times | [`rounds/round-15.md`](rounds/round-15.md) |
| 16 | 2026-09-26 | HEAD `2c5d98e`, desktop 1.3.1 (the cascade) | **NOT CLEAN - six findings, each older than the changes under review**, from the first full sweep of every screen and all five Settings tabs since round 7: D-68 the status bar's engine version and Home's log-path note are not in the mono face; D-69 the Settings Privacy tab carries the Consent notice's words and list, never the dummy's rows (no round had diffed it since 2026-09-07); D-70 Run's hero band is 8 px shorter (`band-tight`); D-71 Home's schedule note sits under the switch, not beside it; D-72 the DUMMY flags section 22 `dev` against the engine; D-73 a URL axis is applied pre-paint and then taken away by the boot pass. What holds: TASK-018 on all four History chips, both version surfaces and the About line at 1.3.1, the 1.3.1 engine's team credits in the Dry-run's own report and log, the plugin-written pre-paint (10 of 10 axes before `<body>`). Signed out, isolated WebView2 profile, two-wire guard: 22 engine calls - 20 `--list`, one `--scan`, one `--dry-run` - Reclaim pressed zero times, 0 bytes reclaimed. Fixes: the app brought to the dummy (D-68 to D-71, D-73) and the dummy's seed corrected (D-72, `AMENDMENTS-3.md`); re-checked in round 17 | [`rounds/round-16.md`](rounds/round-16.md) |
| 17 | 2026-09-26 | HEAD `5dde309`, desktop 1.3.1 - **the commit `desktop-v1.3.1` is tagged on** | **CLEAN** - D-68 to D-73 closed: the engine version and Home's pending note in the mono face on 20 of 20 readings; Privacy 24 / 24 / 24 words and 0 layout mismatches; Run's hero 178 px on both sides; Home's schedule note beside the switch in every Home state; seven `dev` badges on each side; a URL axis shown after the boot pass, never persisted, and yielding to an explicit choice. Round 16's full sweep again (28 states, 224 captures at 1440 and 760, light and dark) finds no new divergence. TASK-018, the 1.3.1 version surfaces, the 1.3.1 engine's team credits, pre-paint (10 of 10 axes before `<body>`) and §10 parity hold. Two observations settled by the main session: Home's 2 px slider offset is declared `prototype` in A-1, and the app-only no-keys sentence was written into the dummy (`AMENDMENTS-3.md`, `settings.html?nokeys=1`). Signed out, isolated WebView2 profile (the owner's 1,354 files identical), two-wire guard: 32 engine calls - 30 `--list`, one `--scan`, one `--dry-run` - Reclaim pressed zero times, 0 bytes reclaimed | [`rounds/round-17.md`](rounds/round-17.md) |
...
| Home's idle-window slider, `input.field[type=range]` (round 17, R17.9(1)) | none - 2 px of margin | `prototype` | Chrome's default margin on a range input, which the dummy's CSS never resets (its one range rule is `.sld input[type="range"]`) and the app's preflight does: the well is 112 px against 114. The static-HTML class round 9 declared for the breadcrumb's `ol` indent (round-09.md:52-54), named here so it is covered by name and not by tolerance |
