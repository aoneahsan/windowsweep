...
| 15 | **Enable Google sign-in on the Supabase project** (after row 23). 🔴 This is a **Web** OAuth client, not a Desktop one - the shape changed with the backend on 2026-09-05. Supabase owns the redirect, so the Cloud Console client's authorised redirect URI is `https://<ref>.supabase.co/auth/v1/callback`, and its id and secret go into Supabase's own Auth -> Providers -> Google form rather than into the app. The app never sees either: it asks Supabase for a provider URL and gets a code back on its loopback listener. Nothing goes in the FilesHub vault for this beyond the project's `url` and publishable key | Cloud Console and Supabase dashboard clicks | phase P6; depends on row 23 | Not started |
