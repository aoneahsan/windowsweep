...
**Whole project: about 77%. The CLI-only scope agreed on 2026-09-03: about 87%. The published CLI on its own:
...
| Desktop app (code, Tauri, Firebase, CI, release) | 24 | 62% | **all eleven screens are built.** Open: the Firebase project (owner row 23); Rust compiled locally (owner row 22 - CI-verified only until then); the WebView2 run-to-verify pass and GATE 4 parity (RW-081); the `desktop-v1.1.0` release (RW-082) |
...
Score = sum(weight x done) / 100 = 25 + 1.2 + 1.2 + 4 + 5 + 7.6 + 4.25 + 8 + 14.88 + 5.5 = **76.63**.
