...
function Invoke-SelfTestExtra {
  ...
  try {
    ...
    # [18] moved to modules/self_test_contract.ps1 at the 1.3.0 cascade (RW-121), to bring this file back
    # under the 500-line ceiling. Same fixture root, same counters, the checks themselves unchanged.
    $contract = Invoke-SelfTestContract -Fixture $fx
    $checks += $contract.Checks; $fails += $contract.Fails
