...
|---|---|---|---|---|
| 11 | 2026-09-14 | desktop 1.2.0 | 🔴 **No verdict.** The drivers ran but none of their stdout was saved (`gate4-evidence/round11/` holds 38 drivers and 16 pairs and no output), so the round cannot be read and is **re-run as round 12**. The two findings written down before the loss are **D-63** and **D-64** | none - the tracker's `knownRisks` carries the incident |
